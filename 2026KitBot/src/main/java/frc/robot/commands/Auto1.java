...
public class Auto1 extends SequentialCommandGroup {
    ...
    public Auto1(CANDriveSubsystem driveSubsystem, CANFuelSubsystem ballSubsystem, double angle1, double angle2, double distance, double speed){
        addCommands(new DriveDistance(driveSubsystem, false, 0.5).withTimeout(0.5), // move back to shoot
        new Launch(ballSubsystem).withTimeout(7), // shoooot
        new DriveDistance(driveSubsystem, true, 0.5).withTimeout(0.5), // return to start
        new TurnToAngle(driveSubsystem, angle1).withTimeout(2), //turn to straight path toward depot
